      ...
      <div class="container">
        <div class="row">
            <div class="col-md-8">
                ...
                $offset=($page-1)*$limit;
                $sql="SELECT * FROM post LEFT JOIN category ON post.category=category.category_id LEFT JOIN user ON post.author=user.user_id WHERE author='{$_GET['author-id']}' LIMIT {$offset},{$limit}";
                $result=mysqli_query($conn,$sql) or die ("Query Failed!");
              $sql1="SELECT * FROM user WHERE user_id={$_GET['author-id']}";
              ...
              $row1=mysqli_fetch_assoc($result1);
                ...
                <?php 
                         ...
                         if(mysqli_num_rows($result2)>0){
                             ...
                             if($page>1){
                                 echo "<li><a href='author.php?author-id={$author_id}&page=".($page - 1 )."'>PREV</a></li>";
                             }
                             for($i=1;$i<=$total_pages;$i++){
                                 if($i==$page){
                                     ...
                                     echo " <li class=".$active."><a href='author.php?author-id={$author_id}&page=$i'>$i</a></li>";
                             }
                             if($page<$total_pages){
                                 echo "<li><a href='author.php?author-id={$author_id}&page=".($page + 1 )."'>NEXT</a></li>";
                             }
                             echo " </ul>";
                         }
